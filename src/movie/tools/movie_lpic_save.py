...
save_path = 'D:/workplace/pythonwork/douban_book_catch_zzq/lpic/movie_pic'
try:
            ...
            for filename in filenames:
                ...
                if filename.endswith(prefix):  # 寻找所有以xlsx结尾的文件
                    ...
                    for col in taglist:
                        ...
                        pic = col[2]
                        if pic.split('/')[-3] == 'ipst':
                            pic_url = pic.replace(pic.split('/')[-3], 'lpst')
                        elif pic.split('/')[-2] == 'spic':
                            pic_url = pic.replace(pic.split('/')[-2], 'lpic')
                        elif pic.split('/')[-2] == 'mpic':
                            pic_url = pic.replace(pic.split('/')[-2], 'lpic')
                        # print(pic_url)
                        movie_url = col[1]
                        ...
                        try:
                            ...
                            f = open('D:/workplace/pythonwork/douban_book_catch_zzq/lpic/movie_pic/wrong.txt', 'a+')
