...
config = {'host': 'localhost',  # 数据库所在的主机地址，默认localhost
          ...
          'port': 3306,  # 默认即为3306
          'database': 'dou_ban_movie',  # 数据库名称（必须是已经存在于数据库中的才能成功）
...
def test_insert():
    dbManager = DbManager()
    dbManager.ExecNonQuery("insert into `movie_detail` (book_name) values ('你哈') ")


# 测试语句
def test_select():
    dbManager = DbManager()
    print(dbManager.execQuery('SELECT * FROM movie_detail limit 10'))
